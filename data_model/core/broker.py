...
class TableBroker(object):
    ...
    @pk.setter
    def pk(self, value):
        raise RuntimeError("Primary key cannot be set")

    def __init__(self):
        super(TableBroker, self).__init__()
        config = get_config()
        config.raise_on_not_set('table_global')

        self._table_name = config.table_global
    ...
    def _get(self, key_dict, **kwargs):
        ...
        key_dict: `dict` contains `global_id` and `chunk_type`, eg.
        \code
                {'global_id': 'xxxxx', 'chunk_type': 'yyyyy'},
        \endcode
        """
    ...
    def batch_delete(self, key_dicts, **kwargs):
        ...
        \code
            [
                {'global_id': 'xxxxx', 'chunk_type': 'yyyyy'},
                {'global_id': 'ooooo', 'chunk_type': 'mmmmm'},
                ...
            ]
        \endcode
        """
    ...
    def _update(self, key_dict, updated_dict, **kwargs):
        """
        Update values defined in `updated_dict` where match condition defined in `key_dict`
        Args:
        ...
        \code
                {'global_id': 'xxxxx', 'chunk_type': 'yyyyy'},
        \endcode
        updated_dict: `dict` contains key-value pairs to be updated, eg.
        \code
        ...
        return self._table.updated_item(
                Key=key_dict,
    ...
    def _batch_get(self, keys, **kwargs):
        ...
        Args:
        keys: List of `dict` contains `global_id` and `chunk_type` for each
        """
        if not keys:
            return None
        ...
        kw = {'RequestItems': {get_config().table_global: kw}}
...
def get_brk():
    if not globals().get(TableBroker.name):
        globals()[TableBroker.name] = TableBroker()
    return globals().get(TableBroker.name)
